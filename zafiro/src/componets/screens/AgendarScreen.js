...
const AgendarScreen = ({ userId }) => {
  ...
  // cambio de colores Botones y listas
  const [buttonColor, setButtonColor] = useState('#285c9b'); // Estado para el color del botón
  const [timeButtonColor, setTimeButtonColor] = useState('#285c9b'); // Estado para el color del botón de la hora
  const [modoReunionColor, setModoReunionColor] = useState('#285c9b'); // Estado para el color del modo de reunión
  ...
  const obtenerNombreUsuario = async () => {
    try {
      const response = await axios.get(`http://192.168.0.3:8080/obtener-nombre-usuario/${userId.userId}`);
  ...
  const getCalendarNameById = (calendarId) => {
    // en caso de error cambiar linea por: 
    const selectedCalendar = calendars.find((calendar) => calendar.id === calendarId);
    // const selectedCalendar = calendars.find((calendar) => calendar.id === 3);
    ...
    if (selectedCalendarId && eventDate && eventTime && selectedAsesor.id_asesor && motivoConsulta.trim() !== '') {
      try {
        ...
        if (
          ...
          try {
            const response = await axios.post('http://192.168.0.3:8080/crear-hora', {
            ...
            // const { data: token } = await axios.post('http://192.168.0.3:8080/get-notification-token', {
            //   userId: userID,
            // });
          ...
          try {
            ...
            const response = await axios.post('http://192.168.0.3:8080/enviar-correo', {
              especialidad: motivoConsulta,
              nombreAsesor: selectedAsesor.nombre,
  ...
  const obtenerAsesores = async () => {
    try {
      ...
      const response = await axios.post('http://192.168.0.3:8080/obtener-asesores', {
        motivoConsulta,
      });
    ...
    <View>
      ...
      <Picker
        ...
        onValueChange={(value) => {
          ...
          setModoReunionColor(value === 'Presencial' || value === 'Virtual' ? '#8e8c84' : '#285c9b');
      ...
      <View style={[styles_Agendar.pickerContainer, { backgroundColor: motivoConsulta !== '' ? '#8e8c84' : '#285c9b' }]}>
        ...
        <Picker
          ...
          itemStyle={{ backgroundColor: motivoConsulta !== '' ? '#8e8c84' : '#285c9b' }} // Cambia el color de los items del Picker
      >
